...
public class CustomTradeManagementMenu extends Menu implements SetModifiersMenu, SetLootPredicatesMenu {
    ...
    public void handleMenu(InventoryClickEvent e) {
        ...
        if (view == View.PROFESSIONS){
            Villager.Profession clickedProfession = clickedItem == null ? null : Catch.catchOrElse(() -> Villager.Profession.valueOf(clickedItem.getMeta().getPersistentDataContainer().get(KEY_PROFESSION, PersistentDataType.STRING)), null);
    ...
    public void setMenuItems() {
        ...
        switch (view){
            case PROFESSIONS -> {
                inventory.setItem(4, professionButton(null));
            ...
            case TRADE -> {
                if (currentTrade != null){
                    ...
                    inventory.setItem(27, new ItemBuilder(Buttons.tradeReputationNegativeModButton).prependLore(String.format("&7Currently &ex%.1f", currentTrade.getNegativeReputationMultiplier()), uses).get());
                    inventory.setItem(29, new ItemBuilder(Buttons.tradePriceDemandMaxButton).prependLore(String.format("&7Currently &e%d", currentTrade.getDemandPriceMax())).get());

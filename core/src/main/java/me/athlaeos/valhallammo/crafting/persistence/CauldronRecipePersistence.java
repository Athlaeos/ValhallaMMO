...
public class CauldronRecipePersistence extends RecipePersistence {
    public CauldronRecipePersistence(Gson gson) {
        super(gson);
    }
    ...
    public Map<String, ValhallaRecipe> getRecipesFromFile(File f){
        ...
        if (f.exists()){
            try (BufferedReader recipesReader = new BufferedReader(new FileReader(f, StandardCharsets.UTF_8))){
                DynamicCauldronRecipe[] collectedRecipes = gson.fromJson(recipesReader, DynamicCauldronRecipe[].class);
                for (DynamicCauldronRecipe recipe : collectedRecipes) if (recipe != null) recipes.put(recipe.getName(), recipe);
    ...
    public void saveRecipesToFile(String fileName, Collection<? extends ValhallaRecipe> recipes){
        ...
        recipes = new ArrayList<>(recipes.stream().filter(r -> r instanceof DynamicCauldronRecipe).toList());
        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(f, StandardCharsets.UTF_8))){
            JsonElement element = gson.toJsonTree(new ArrayList<>(recipes), new TypeToken<ArrayList<DynamicCauldronRecipe>>(){}.getType());
            gson.toJson(element, writer);
            writer.flush();
        } catch (IOException | JsonSyntaxException exception){
            ValhallaMMO.logSevere("Could not save cauldron recipes file " + fileName+ ", " + exception.getMessage());
        }
    }
}

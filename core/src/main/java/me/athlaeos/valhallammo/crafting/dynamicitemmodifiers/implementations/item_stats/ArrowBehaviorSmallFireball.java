...
public class ArrowBehaviorSmallFireball extends DynamicItemModifier {
    private double radius = 3;
    private boolean incendiary = false;

    public ArrowBehaviorSmallFireball(String name) {
        super(name);
    }

    @Override
    public void processItem(ModifierContext context) {
        ArrowBehaviorRegistry.addBehavior(context.getItem().getMeta(), ArrowBehaviorRegistry.FIREBALL_SMALL.getName(), radius, incendiary ? 1 : 0);
    }
    ...
    public String getDisplayName() {
        return "&6Arrow Behavior: Small Fireball";
    }

    @Override
    public String getDescription() {
        return "&fTurns the arrow into a small fireball when shot";
    }

    @Override
    public String getActiveDescription() {
        return String.format("&fTurns the arrow into a small fireball when shot. /n&eBlast Radius: %.1f/n&eIncendiary: %s", radius, incendiary ? "Yes" : "No");
    }

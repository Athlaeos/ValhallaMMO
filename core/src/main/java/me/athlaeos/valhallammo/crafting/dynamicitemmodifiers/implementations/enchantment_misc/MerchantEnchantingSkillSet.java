...
public class MerchantEnchantingSkillSet extends DynamicItemModifier {
    ...
    public void onButtonPress(InventoryClickEvent e, int button) {
        if (button == 2) shouldScaleWithStats = !shouldScaleWithStats;
        else if (button == 10) qualityMap.put(MerchantLevel.NOVICE, Math.max(0, qualityMap.getOrDefault(MerchantLevel.NOVICE, 30) + ((e.isLeftClick() ? 1 : -1) * (e.isShiftClick() ? 10 : 1))));
        ...
        return new Pair<>(10,
                ...
                new Pair<>(2,
                        new ItemBuilder(Material.REDSTONE_TORCH)
                                .name("&eShould this scale with player stats?")
                                .lore("&fSet to &e" + (shouldScaleWithStats ? "Yes" : "No"),
                                        "&fIf enabled, this quality can be further",
                                        "&fmodified based on the player's ",
                                        "&f'merchant skill multiplier' stat.",
                                        "&fOtherwise, the quality stays as is",
                                        "",
                                        "&6Click to toggle")
                                .get())
        ));
    ...
    public void setShouldScaleWithStats(boolean shouldScaleWithStats) {
        this.shouldScaleWithStats = shouldScaleWithStats;
    }

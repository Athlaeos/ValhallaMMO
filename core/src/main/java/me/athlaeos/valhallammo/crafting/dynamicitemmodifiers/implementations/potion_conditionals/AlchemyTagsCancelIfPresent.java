...
public class AlchemyTagsCancelIfPresent extends DynamicItemModifier {
    private final Collection<Integer> tags = new HashSet<>();
    private int tag = 0;

    public AlchemyTagsCancelIfPresent(String name) {
        super(name);
    }
    ...
    public String getDisplayName() {
        return "&7Potion Tags (NONE MATCH)";
    }

    @Override
    public String getDescription() {
        return "&fIf the item has any of the given tags, recipe is cancelled with an error message forwarded to the player. Error messages defined in skills/alchemy.yml";
    }
    ...
    public DynamicItemModifier copy() {
        AlchemyTagsCancelIfPresent m = new AlchemyTagsCancelIfPresent(getName());
        m.setTag(this.tag);

...
public class ImmersiveRecipePersistence extends RecipePersistence {
    public ImmersiveRecipePersistence(Gson gson) {
        super(gson);
    }
    ...
    public Map<String, ValhallaRecipe> getRecipesFromFile(File f){
        ...
        if (f.exists()){
            try (BufferedReader recipesReader = new BufferedReader(new FileReader(f, StandardCharsets.UTF_8))){
                ImmersiveCraftingRecipe[] collectedRecipes = gson.fromJson(recipesReader, ImmersiveCraftingRecipe[].class);
                for (ImmersiveCraftingRecipe recipe : collectedRecipes) if (recipe != null) recipes.put(recipe.getName(), recipe);
    ...
    public void saveRecipesToFile(String fileName, Collection<? extends ValhallaRecipe> recipes){
        ...
        recipes = new ArrayList<>(recipes.stream().filter(r -> r instanceof ImmersiveCraftingRecipe).toList());
        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(f, StandardCharsets.UTF_8))){
            JsonElement element = gson.toJsonTree(new ArrayList<>(recipes), new TypeToken<ArrayList<ImmersiveCraftingRecipe>>(){}.getType());
            gson.toJson(element, writer);
            writer.flush();
        } catch (IOException | JsonSyntaxException exception){
            ValhallaMMO.logSevere("Could not save immersive recipes file " + fileName+ ", " + exception.getMessage());
        }
    }
}

...
public class BrewingRecipePersistence extends RecipePersistence {
    public BrewingRecipePersistence(Gson gson) {
        super(gson);
    }
    ...
    public Map<String, ValhallaRecipe> getRecipesFromFile(File f){
        ...
        if (f.exists()){
            try (BufferedReader recipesReader = new BufferedReader(new FileReader(f, StandardCharsets.UTF_8))){
                DynamicBrewingRecipe[] collectedRecipes = gson.fromJson(recipesReader, DynamicBrewingRecipe[].class);
                for (DynamicBrewingRecipe recipe : collectedRecipes) if (recipe != null) recipes.put(recipe.getName(), recipe);
    ...
    public void saveRecipesToFile(String fileName, Collection<? extends ValhallaRecipe> recipes){
        ...
        recipes = new ArrayList<>(recipes.stream().filter(r -> r instanceof DynamicBrewingRecipe).toList());
        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(f, StandardCharsets.UTF_8))){
            JsonElement element = gson.toJsonTree(new ArrayList<>(recipes), new TypeToken<ArrayList<DynamicBrewingRecipe>>(){}.getType());
            gson.toJson(element, writer);
            writer.flush();
        } catch (IOException | JsonSyntaxException exception){
            ValhallaMMO.logSevere("Could not save brewing recipes file " + fileName+ ", " + exception.getMessage());
        }
    }
}

...
public class ReplacementTableSelectionMenu extends Menu {
            ...
            switch (action){
                ...
                case "createNewButton" -> {
                    ...
                    Questionnaire questionnaire = new Questionnaire((Player) e.getWhoClicked(), null, null,
                            new Question("&fWhat should the replacement table's key be? (type in chat, or 'cancel' to cancel)", s -> !LootTableRegistry.getReplacementTables().containsKey(s), "&cReplacement table with this key already exists! Try again")
                    ) {
                        @Override
                        public Action<Player> getOnFinish() {
                            ...
                            return (p) -> {
                                ...
                                if (answer.contains("cancel")) playerMenuUtility.getPreviousMenu().open();
                                    ...
                                    Utils.sendMessage(getWho(), "&cReplacement table key already exists!");
                                else {
    ...
    public void setMenuItems() {
        ...
        for (ReplacementTable table : LootTableRegistry.getReplacementTables().values()){
            ...
            if (table.getReplacementPools().isEmpty()) icon.appendLore("&cNo replacement pools set, replaces nothing");
            else icon.appendLore("&aSelecting replacement from " + table.getReplacementPools().size() + " pools");

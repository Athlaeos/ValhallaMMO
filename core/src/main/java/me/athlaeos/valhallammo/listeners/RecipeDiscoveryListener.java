...
public class RecipeDiscoveryListener implements Listener {
    ...
    public void onPlayerJoin(PlayerJoinEvent e){
        ...
        for (DynamicGridRecipe recipe : CustomRecipeRegistry.getGridRecipes().values()){
            if (!recipe.isHiddenFromBook() && (recipe.isUnlockedForEveryone() || profile.getUnlockedRecipes().contains(recipe.getName()) || allPermission || e.getPlayer().hasPermission("valhalla.recipe." + recipe.getName()))) e.getPlayer().discoverRecipe(recipe.getKey()); // all recipes have a shaped variant because these display properly in the recipe book
        ...
        for (DynamicSmithingRecipe recipe : CustomRecipeRegistry.getSmithingRecipes().values()){
            if (!recipe.isHiddenFromBook() && (recipe.isUnlockedForEveryone() || profile.getUnlockedRecipes().contains(recipe.getName()) || allPermission || e.getPlayer().hasPermission("valhalla.recipe." + recipe.getName()))) e.getPlayer().discoverRecipe(recipe.getKey());
            else e.getPlayer().undiscoverRecipe(recipe.getKey());
        }

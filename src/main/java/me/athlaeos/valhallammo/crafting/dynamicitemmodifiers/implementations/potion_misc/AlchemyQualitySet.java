...
public class AlchemyQualitySet extends DynamicItemModifier {
    private int quality = 50;

    public AlchemyQualitySet(String name) {
        super(name);
    }

    @Override
    public void processItem(Player crafter, ItemBuilder outputItem, boolean use, boolean validate, int timesExecuted) {
        AlchemyItemPropertyManager.setQuality(outputItem.getMeta(), quality);
    }
    ...
    public String getDisplayName() {
        return "&dAlchemy Quality (NUMERIC)";
    }
    ...
    public String parseCommand(CommandSender executor, String[] args) {
        if (args.length != 2) return "One argument is expected: an integer";
        try {

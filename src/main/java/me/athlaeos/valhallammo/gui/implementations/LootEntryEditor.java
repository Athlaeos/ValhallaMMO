...
public class LootEntryEditor extends Menu implements SetModifiersMenu, SetLootPredicatesMenu {
    ...
    public void setMenuItems() {
        ...
        if (!entry.isGuaranteedPresent()) inventory.setItem(chanceWeightLuckIndex1, pool.isWeighted() ? new ItemBuilder(setBonusWeightButton1).name("&eBonus Weight: " + entry.getWeight()).get() : new ItemBuilder(setBonusChanceButton1).name(String.format("&eBonus Drop Chance: %.4f%%", entry.getChance() * 100)).get());
        if (!entry.isGuaranteedPresent()) inventory.setItem(chanceWeightLuckIndex2, pool.isWeighted() ? new ItemBuilder(setBonusWeightButton2).name("&eBonus Weight: " + entry.getWeight()).get() : new ItemBuilder(setBonusChanceButton2).name(String.format("&eBonus Drop Chance: %.4f%%", entry.getChance() * 100)).get());
        if (!entry.isGuaranteedPresent()) inventory.setItem(chanceWeightLuckIndex3, pool.isWeighted() ? new ItemBuilder(setBonusWeightButton3).name("&eBonus Weight: " + entry.getWeight()).get() : new ItemBuilder(setBonusChanceButton3).name(String.format("&eBonus Drop Chance: %.4f%%", entry.getChance() * 100)).get());
    }
